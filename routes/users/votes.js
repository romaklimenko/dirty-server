...
router.get('/api/users/:username/votes/', async (req, res) => {
  try {
    ...
    if (!username || username === '') {
      ...
      const posts_response = await (await fetch(`https://d3.ru/api/users/${username}/posts/`)).json();
      ...
      const comments_response = await (await fetch(`https://d3.ru/api/users/${username}/comments/`)).json();
      ...
      let activities = posts_response.posts
        .concat(comments_response.comments)
      ...
      for (let i = 0; i < activities.length; i++) {
        ...
        for (let j = 0; j < votes.length; j++) {
          ...
          if (vote.vote > 0) {
            voters[vote.user.login].pros += vote.vote;
          }
          else {
            voters[vote.user.login].cons += vote.vote;
          }
        }
      }
